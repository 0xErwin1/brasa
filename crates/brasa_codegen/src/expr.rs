...
fn field(f: &mut FuncCx, id: ExprId, recv: ExprId, name: &str, span: Span) {
    ...
    // the `fs` `Walk` (BRS-66) and `Stat`, the `http` `Response`
    // (BRS-113), or the `NativeError` a `catch` arm binds (BRS-135) —
    // is a receiver-only builtin call yielding the field value
    // directly, rather than a bound method. `Response::header` is
    // deliberately absent: it takes an argument, so it IS a method and
    // binds like one.
    let native_field = match f.cx.types.expr_types.get(&recv) {
        ...
        Some(Type::Stat) => matches!(
            name,
            "size" | "modifiedMillis" | "isFile?" | "isDir?" | "isSymlink?"
        ),
